...
import (
	...
	harmoniaApp "github.com/zenanet-network/harmonia/app"
)
...
func TestModulesStreamedGenesisExport(t *testing.T) {
	...
	happ := harmoniaApp.NewHarmoniaApp(logger, db)

	genDoc, err := tmTypes.GenesisDocFromFile("./testdata/dump-genesis.json")
	require.NoError(t, err)

	var genesisState harmoniaApp.GenesisState
	...
	for idx, record := range eventRecords {
		...
		require.NotEmpty(t, eventRecord["eirene_chain_id"])
		require.NotEmpty(t, eventRecord["record_time"])
	}

	eirene, err := traversePath(appState, "eirene")
	require.NoError(t, err)

	spans, ok := eirene["spans"].([]interface{})
	...
	for idx, span := range spans {
		...
		require.NotEmpty(t, spanMap["eirene_chain_id"])
	}
}
