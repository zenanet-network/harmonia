...
func (ak AccountKeeper) NewAccountWithAddress(ctx sdk.Context, addr hmTypes.HarmoniaAddress) types.Account {
	acc := ak.proto()
...
func (ak AccountKeeper) GetAccount(ctx sdk.Context, addr hmTypes.HarmoniaAddress) types.Account {
...
func (ak AccountKeeper) GetPubKey(ctx sdk.Context, addr hmTypes.HarmoniaAddress) (crypto.PubKey, sdk.Error) {
...
func (ak AccountKeeper) GetSequence(ctx sdk.Context, addr hmTypes.HarmoniaAddress) (uint64, sdk.Error) {
	acc := ak.GetAccount(ctx, addr)
	if acc == nil {
...
func (ak AccountKeeper) GetBlockProposer(ctx sdk.Context) (hmTypes.HarmoniaAddress, bool) {
	store := ctx.KVStore(ak.key)
...
func (ak AccountKeeper) SetBlockProposer(ctx sdk.Context, addr hmTypes.HarmoniaAddress) {
	store := ctx.KVStore(ak.key)
