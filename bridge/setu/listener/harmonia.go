...
const (
	harmoniaLastBlockKey = "harmonia-last-block" // storage key
)

// HarmoniaListener - Listens to and process events from heimdall
type HarmoniaListener struct {
	BaseListener
}

// NewHarmoniaListener - constructor func
func NewHarmoniaListener() *HarmoniaListener {
	return &HarmoniaListener{}
}

// Start starts new block subscription
func (hl *HarmoniaListener) Start() error {
...
// ProcessHeader -
func (hl *HarmoniaListener) ProcessHeader(_ *blockHeader) {

}

// StartPolling - starts polling for heimdall events
func (hl *HarmoniaListener) StartPolling(ctx context.Context, pollInterval time.Duration, _ *big.Int) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				...
				if err := hl.storageClient.Put([]byte(harmoniaLastBlockKey), []byte(strconv.FormatUint(toBlock, 10)), nil); err != nil {
...
func (hl *HarmoniaListener) fetchFromAndToBlock() (uint64, uint64, error) {
	...
	hasLastBlock, _ := hl.storageClient.Has([]byte(harmoniaLastBlockKey), nil)
	if hasLastBlock {
		lastBlockBytes, err := hl.storageClient.Get([]byte(harmoniaLastBlockKey), nil)
		if err != nil {
...
// ProcessBlockEvent - process Blockevents (BeginBlock, EndBlock events) from harmonia.
func (hl *HarmoniaListener) ProcessBlockEvent(event sdk.StringEvent, blockHeight int64) {
	hl.Logger.Info("Received block event from Harmonia", "eventType", event.Type)
	...
	case slashingTypes.EventTypeSlashLimit:
		hl.sendBlockTask("sendTickToHarmonia", eventBytes, blockHeight)
...
func (hl *HarmoniaListener) sendBlockTask(taskName string, eventBytes []byte, blockHeight int64) {
