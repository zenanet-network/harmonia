...
import (
	...
	hmClient "github.com/zenanet-network/harmonia/client"
...
func CreateNewStateRecord(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// bor chain id
			eireneChainID := viper.GetString(FlagEireneChainId)
			if eireneChainID == "" {
				return fmt.Errorf("EireneChainID cannot be empty")
			}
	...
	cmd.Flags().String(FlagEireneChainID, "", "--eirene-chain-id=<eirene-chain-id>")
	...
	if err := cmd.MarkFlagRequired(FlagEireneChainID); err != nil {
