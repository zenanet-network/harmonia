...
import (
	...
	types "github.com/zenanet-network/harmonia/types/error"
)
...
func TestPermCheck(t *testing.T) {
	...
	tc := []struct {
		...
		{
			filePath:  "/tmp/harmonia_test/test.json",
			perm:      0777,
			validPerm: 0600,
			expErr:    types.InvalidPermissionsError{File: "/tmp/harmonia_test/test.json", Perm: 0600},
